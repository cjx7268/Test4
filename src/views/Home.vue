        ...
        .then((response)=>{
          ...
          for(let i = 0;i<data2.result.length;i++){
            articles.push({
              ...
              articleId: data2.result[i].post_id,
              title: data2.result[i].post_theme,
              content: '',
              date: data2.result[i].post_time,
              likeUsers: data2.result[i].post_favor_amount,
              comments: data2.result[i].post_comment_amount,
              uname:''
            })
          ...
          console.log(ls)
          // this.$store.commit()
          // this.$store.commit('UPDATE_ARTICLES', userArticles)
          ////其他操作
        })
        .catch((error)=>{
