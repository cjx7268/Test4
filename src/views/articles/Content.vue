...
export default {
  ...
  created() {
    ...
    this.$axios.get(global_.Url + 'posts/postContent',{
      params:{
        UID: "1",
        post_id:this.$route.params.articleId,
      }
    })
      .then((response)=>{
        ...
        for(let i = 0 ; i < data2.length ; i++){
          contents.push({
            uid : parseInt(data2[i].UID),
            content : data2[i].floor_content,
            num : data2[i].floor_num,
            time : data2[i].floor_time,
        ...
        // this.$set(this.contents[i],uid,data2.UID)
        this.$store.commit('UPDATE_CONTENTS',contents)
        var myDate = new Date();
        console.log(myDate.toLocaleString());
        this.content2=this.$store.getters.getContents(articleId)
        console.log(this.content2)
        this.content=this.content2[0].content
        var comments2 = []
        for(let i = 1 ; i < this.content2.length ; i++){
          comments2.push({
            uid : this.content2[i].uid,
            content : this.content2[i].content,
            commentId : this.content2[i].num-1,
            time : this.content2[i].time,
            uname : 'cjx040',
            uavatar : 'https://api.adorable.io/avatars/200'
            })
        }
    ...
    // this.content2=this.$store.state.contents
    // console.log(this.content2)
    ...
    comment() {
      ...
      this.$axios.get(global_.Url + 'posts/followPost',{
        params:{
          UID: "1",
          post_id:this.$route.params.articleId,
          floor_content:this.commentMarkdown,
          ////时间在后端生成， 层数需要获取
        }
      })
        .then((response)=>{
      ...
      if (this.commentMarkdown && this.commentMarkdown.trim() !== '') {
        ...
        comments.push({
          uid : '1',
          content : this.commentsMarkdown,
          num : this.content2.length,
          time : '',
          uname: 'cjx040',
          uavatar: ""
        })
